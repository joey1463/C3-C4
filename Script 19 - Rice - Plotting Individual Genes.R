...
chosen_gene<-"LOC-Os12g17600" # Rubisco 
...
chosen_gene<-"LOC-Os06g24070" # GLK1 # better in CI
chosen_gene<-"LOC-Os01g13740" # GLK2 # good in CI
...
ggplot(data=to_plot_2, aes(x=time, y=expression, group=tissue)) + geom_line(aes(color=tissue)) + geom_point(size=0) + theme_bw() + theme(axis.text=element_text(size=10),axis.title=element_text(size=15)) + xlab("time (h)") + ylab("normalized expression")   +  theme(plot.margin = margin(1,1,1.5,1.2, "cm"))
