# This script will take read counts for 48h time point sci-RNA-seq3 experiments and assemble them into a counts matrix for Seurat. This does it for each species seperately. 
...
for (i in 1:42) {rice_matrix_lists[[i]] <- generate_matrix(gene_count_folder=paste("/gale/netapp/home/jswift/analysis/CI_Light/CI_Light_rice_48hr/split_",i,"/Arabidopsis_gene_count",sep=''), name_start=1,name_stop=14)}
for (i in 1:14) {sorghum_matrix_lists[[i]] <- generate_matrix(gene_count_folder=paste("/gale/netapp/home/jswift/analysis/CI_Light/CI_Light_sorghum_48hr/split_",i,"/Arabidopsis_gene_count",sep=''), name_start=1,name_stop=16)}

print('no. nuclei/combinations in each rice split (this is raw count; before low count removal)')
for (i in 1:42) {print(length(rice_matrix_lists[[i]][1,]))}
print('no. nuclei/combinations in each sorghum split (this is raw count; before low count removal)')
for (i in 1:14) {print(length(sorghum_matrix_lists[[i]][1,]))}

# Make all rows the same
all_rice_gene_names <- NA
for (i in 1:42) {all_rice_gene_names <- c(all_rice_gene_names, rownames(rice_matrix_lists[[i]]))}
all_rice_gene_names <- unique(all_rice_gene_names[-1])

all_sorghum_gene_names <- NA
for (i in 1:14) {all_sorghum_gene_names <- c(all_sorghum_gene_names, rownames(sorghum_matrix_lists[[i]]))}
...
for (i in 1:42) {rice_matrix_lists_sized[[i]] <- adding_missing_genes(all_rice_gene_names, rice_matrix_lists[[i]])}

sorghum_matrix_lists_sized<-list(NA)
for (i in 1:14) {sorghum_matrix_lists_sized[[i]] <- adding_missing_genes(all_sorghum_gene_names, sorghum_matrix_lists[[i]])}

RT_Identifier_Rice<-read.table("/gale/ddn/ddn_neomorph/jswift/analysis/R_Light/text_files/Rice_Identifier_48hr.txt",header=TRUE, sep="\t") 
RT_Identifier_Sorghum<-read.table("/gale/ddn/ddn_neomorph/jswift/analysis/R_Light/text_files/Sorghum_Identifier_48hr.txt",header=TRUE, sep="\t") 

get_new_identifiers<-function(input_matrix, input_identifier){

  RT_Identifier<-input_identifier
...
for (i in 1:42) {colnames(rice_matrix_lists_sized[[i]]) <- get_new_identifiers(rice_matrix_lists_sized[[i]], RT_Identifier_Rice)}
for (i in 1:14) {colnames(sorghum_matrix_lists_sized[[i]]) <- get_new_identifiers(sorghum_matrix_lists_sized[[i]], RT_Identifier_Sorghum)}

# Checking No Duplicate Names Rice
print('checking rice no duplicates (last number should be zero)')
all_well_names_rice <- NA
for (i in 1:42) {all_well_names_rice <- c(all_well_names_rice, colnames(rice_matrix_lists_sized[[i]]))}
...
for (i in 1:14) {all_well_names_sorghum <- c(all_well_names_sorghum, colnames(sorghum_matrix_lists_sized[[i]]))}
...
for (i in 1:42) {full_matrix_rice <- cbind(full_matrix_rice, rice_matrix_lists_sized[[i]]) }
full_matrix_rice_48hr <-full_matrix_rice[,-1]

full_matrix_sorghum <- matrix(NA,nrow=length(all_sorghum_gene_names),ncol=1)
for (i in 1:14) {full_matrix_sorghum <- cbind(full_matrix_sorghum, sorghum_matrix_lists_sized[[i]]) }
full_matrix_sorghum_48hr <-full_matrix_sorghum [,-1]

save(full_matrix_rice_48hr, file = "CI_full_matrix_rice_48hr.RData")
save(full_matrix_sorghum_48hr, file = "CI_full_matrix_sorghum_48hr.RData")
