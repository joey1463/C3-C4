...
ggplot(data=to_plot_2, aes(x=time, y=expression, group=tissue))  + geom_point(size=2,aes(color=tissue))   + theme_bw() + theme(legend.position = "none") + ylab("normalized counts") + geom_smooth(method = "loess", se = FALSE, aes(color=tissue), size=1) + xlab("time (h)") + scale_color_manual(values=c("blue3", "green4")) +  theme(plot.margin = margin(1,1,1.5,1.2, "cm")) + theme(axis.text=element_text(size=12),axis.title=element_text(size=14))
...
to_plot_2[,3]<-c('bundle_sheath','epidermis','guard','mesophyll','phloem','xylem')    
colnames(to_plot_2)<-c('expression','time','tissue')
ggplot(data=to_plot_2, aes(x=time, y=expression, group=tissue)) + geom_line(aes(color=tissue)) + geom_point(size=0) + theme_bw() + theme(axis.text=element_text(size=10),axis.title=element_text(size=15)) +  theme(plot.margin = margin(1,1,1.5,1.2, "cm"))  + xlab("time (h)") + ylab("normalized expression") + theme(legend.position="none")
