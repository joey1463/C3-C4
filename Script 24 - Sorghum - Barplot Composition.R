...
raw_table<-table(as.character(sorghum_combined@active.ident), paste(sorghum_combined$assay_type, sorghum_combined$light_time, sep="_"))
...
colnames(normalized_table_1)<-c("timepoint","day","value")
normalized_table_1[,1]<-as.factor(normalized_table_1[,1])

b1 <- ggplot(normalized_table_1, aes(fill=timepoint, y=value, x=as.factor(day))) + geom_bar(position="stack", stat="identity")
