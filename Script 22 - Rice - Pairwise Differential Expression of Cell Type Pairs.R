...
Iterate_ANCOVA<-function(cell_type_1, cell_type_2){
  time_aggregate<-rice_time_aggregate
  ...
  time_aggregate_subset<-time_aggregate_subset[rowSums(time_aggregate_subset == 0) < 6, ] # restricting drop out
  ...
  normalized_reads <- counts(estimateSizeFactors(DEmat), normalized=TRUE) 
  ...
  # Add on tissue specific genes only
  ...
  # Final Export  
  Rice_ANCOVA_all_Tissue<-unique(c(rownames(Tissue_Significant_updated),rownames(Tissue_Time_Significant_updated)))
  Rice_ANCOVA_all_Tissue}

cell_types<-c("mesophyll","bundle_sheath","guard","epidermis","phloem","xylem")

possible_pairs<-combn(cell_types,2)

Rice_Pair_Permutations<-list(NA)
for (i in 1:15){ 
  Rice_Pair_Permutations[[i]]<-Iterate_ANCOVA(possible_pairs[1,i],possible_pairs[2,i])}

setwd("~/Desktop/Salk/Project - Light/R files")
save(Rice_Pair_Permutations, file = "L3_Rice_Pair_Permutations.RData")
