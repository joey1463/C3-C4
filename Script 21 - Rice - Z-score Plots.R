...
DE_Significant<-subset(DE_Significant, DE_Significant$padj<2 & DE_Significant$log2FoldChange>0) # change to "<0" for downregualted genes 
...
table(cutree(clustering_1, h=0.65))
cut_groups<-cutree(clustering_1, h=0.65) 
table(cut_groups)
...
cell_type<-substr(factored,11,25)
cell_type<-str_remove(cell_type, "e_") 
cell_type<-str_remove(cell_type, "_") 
cell_type<-str_replace(cell_type, "bundlsheath","bundlesheath") 
plot_matrix[,4]<-time
...
plot_list[[i]]<-ggplot(data=plot_matrix, aes(x=time, y=expression, group=gene_tissue)) + geom_smooth(method = "loess", se = FALSE, aes(color=cell_type)) +   scale_x_continuous(breaks=c(0,4,8,12)) + theme_bw()  + xlab("time (h)") + ylab("Z-score") +labs(title=title) + theme(legend.position = "none") }  #

# Figure 3
grid.arrange(plot_list[[order[1]]],plot_list[[order[6]]],plot_list[[order[5]]],plot_list[[order[2]]],plot_list[[order[3]]],plot_list[[order[4]]],nrow=2) 

# Supplementary Figure
grid.arrange(plot_list[[order[1]]],plot_list[[order[2]]],plot_list[[order[3]]],plot_list[[order[6]]],plot_list[[order[5]]],plot_list[[order[4]]],nrow=2) 
